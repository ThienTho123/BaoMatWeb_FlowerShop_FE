...
const OrderShipped = () => {
  ...
  const getCommentInfo = async () => {
    try {
      const response = await axios.get(
        "https://localhost:8080/shipperaccount/ordership",
        {
          headers: {
      ...
      const updatedComment = rawComment.map((item, index) => ({
        ...
        dateStart: dayjs(item.shipping.startDate).format("YYYY-MM-DD HH:mm:ss"),
  ...
  return (
    <div className="send-comment-container">
      <h2 className="comment-title">Các đơn hàng đã giao hoặc bị hủy</h2>
