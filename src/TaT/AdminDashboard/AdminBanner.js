...
const AdminBanner = () => {
    ...
    const fetchBanners = async () => {
      try {
        const response = await fetch(
          "https://localhost:8080/api/v1/admin/banner",
          {
            headers: {
  ...
  const handleSave = async (id, bannerData) => {
    try {
      const bannerToUpdate = bannerList.find(
  ...
  const handleSelectChange = (type, value) => {
    setNewBanner((prev) => ({
      ...prev,
      flower:
        type === "flower" ? { flowerID: value || null } : { flowerID: null },
      category:
        type === "category"
          ? { categoryID: value || null }
          : { categoryID: null },
      purpose:
        type === "purpose" ? { purposeID: value || null } : { purposeID: null },
    }));
  };
  ...
  return (
    <div className="admin-ql-container">
      ...
      <div>
        ...
        {imageUrl && (
          <img src={imageUrl} alt="Banner Avatar" style={{ width: 100 }} />
        )}
        <br />
        ...
        <select
          ...
          disabled={
            newBanner.category?.categoryID || newBanner.purpose?.purposeID
          }
        >
        ...
        <select
          ...
          disabled={
            newBanner.flower?.flowerID || newBanner.category?.categoryID
          }
        >
        ...
        {editingBannerId ? (
          ...
          <button onClick={() => handleSave(editingBannerId, newBanner)}>
            Lưu
          </button>
        ) : (
              ...
              <tr key={banner.bannerID}>
                ...
                <td>
                  {editingBannerId === banner.bannerID ? (
                    <>
                      <button
                        onClick={() => handleSave(banner.bannerID, newBanner)}
                      >
                        Lưu
                      </button>
