    ...
    const fetchOrderDetails = async () => {
      try {
        const response = await fetch(
          `https://localhost:8080/api/v1/admin/orderdetail`,
          {
            headers: {
            ...
            {orderDetails.map((orderDetail) => (
              <tr key={orderDetail.orderdetailID}>
                ...
                <td>
                  {editingOrderDetailId === orderDetail.orderdetailID ? (
                    <button
                      onClick={() => handleSave(orderDetail.orderdetailID)}
                    >
                      Lưu
                    </button>
                  ) : (
                    <>
                      <button
                        onClick={() =>
                          setEditingOrderDetailId(orderDetail.orderdetailID)
                        }
                      >
                        Chỉnh Sửa
                      </button>
                      <button
                        onClick={() =>
                          handleSoftDelete(orderDetail.orderdetailID)
                        }
                      >
                        Vô hiệu hóa
                      </button>
