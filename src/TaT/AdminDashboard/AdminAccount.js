...
const AdminAccount = () => {
  ...
  const handleSave = async (accountID, account) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAccounts(accounts.map((a) => (a.accountID === accountID ? updatedAccount : a)));
  ...
  const handleCreate = async () => {
    ...
    if (newAccount.email.length < 2 || !/^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+$/.test(newAccount.email)) {
      errorMessage += "Email không hợp lệ.\n";
    }
    if (newAccount.phoneNumber.length < 8 || newAccount.phoneNumber.length > 10) {
    ...
    if (accountID) {
      setAccounts(
        accounts.map((account) =>
          account.accountID === accountID ? { ...account, [field]: value } : account
        )
      );
    } else {
      setNewAccount((prev) => ({
